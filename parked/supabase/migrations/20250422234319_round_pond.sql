/*
  # Add debug logging and fix team join function
  
  1. Changes
    - Add explicit error handling for each step of the join process
    - Add additional COALESCE checks for all string concatenations
    - Ensure all variables are properly initialized
    - Fix potential issues with team name retrieval
...
BEGIN
  ...
  BEGIN
    ...
    SELECT 
      ...
      v_user_name || ' has requested to join ' || v_team_name,
      jsonb_build_object(
