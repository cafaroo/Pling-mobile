/*
  # Fix team join function with proper null handling
  
  1. Changes
    - Add explicit filter for non-null user_id in notifications insert
    - Use jsonb_build_object instead of json_build_object for consistency
    - Add additional error handling and validation
    - Ensure message is never null with COALESCE
...
BEGIN
  ...
  v_notification_message := COALESCE(v_user_name, 'A new user') || ' has requested to join ' || COALESCE(v_team_name, 'the team');
