...
/**
 * Strategi för att hantera begränsningar av antalet rapporter en organisation kan ha.
 */
export class ReportLimitStrategy extends BaseResourceLimitStrategy {
  constructor(subscriptionAdapter: SubscriptionAdapter, fallbackLimit: number = 2) {
    super(subscriptionAdapter, 'reports', fallbackLimit);
  }

  /**
   * Kontrollerar om fler rapporter kan skapas i en organisation.
   * 
   * @param organizationId - ID för den aktuella organisationen
   * @param currentCount - Nuvarande antal rapporter
   * @param addCount - Antal rapporter som försöker läggas till
  ...
  async isActionAllowed(
    ...
    const resourceLabel = ResourceTypeLabels[ResourceType.REPORT];
