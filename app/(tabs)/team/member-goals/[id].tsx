...
import { Target, ArrowLeft, Trash, CircleCheck as CheckCircle, Archive, Plus } from 'lucide-react-native';
...
export default function MemberGoalDetailScreen() {
  ...
  useEffect(() => {
    if (!id) {
      router.replace('/team/member-goals');
      return;
    }
  ...
  const handleDeleteGoal = async () => {
    ...
    Alert.alert(
      'Delete Goal',
      'Are you sure you want to delete this member goal? This action cannot be undone.',
      [
        {
          text: 'Cancel',
          style: 'cancel'
  ...
  return (
    <Container>
      ...
      />
      <Header 
        title={`${goal.assigneeName}'s Goal`}
        icon={Target}
        leftIcon={ArrowLeft}
        onLeftIconPress={() => router.back()}
      />
