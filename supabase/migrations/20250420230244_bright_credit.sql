/*
  # Fix get_available_features function
  
  1. Changes
    - Use proper record type handling for feature arrays
    - Fix column access syntax
    - Maintain same functionality with corrected implementation
    
  2. Security
    - Maintain SECURITY DEFINER
    - Keep stable execution context
*/

-- Create composite type for feature definition
DO $$ BEGIN
  CREATE TYPE feature_definition AS (
    feature text,
    limit_type text,
    limit_value integer
  );
...
BEGIN
  -- Assign features based on tier
  features := CASE tier_name
    WHEN 'free' THEN 
      ARRAY[
        ROW('team_members', 'count', 3)::feature_definition,
        ROW('monthly_sales', 'count', 50)::feature_definition,
        ROW('basic_analytics', 'boolean', 1)::feature_definition
      ]
    WHEN 'pro' THEN 
      ARRAY[
        ROW('team_members', 'count', 10)::feature_definition,
        ROW('monthly_sales', 'count', -1)::feature_definition,
        ROW('advanced_analytics', 'boolean', 1)::feature_definition,
        ROW('export', 'boolean', 1)::feature_definition,
        ROW('team_chat', 'boolean', 1)::feature_definition,
        ROW('custom_competitions', 'boolean', 1)::feature_definition
      ]
    WHEN 'business' THEN 
      ARRAY[
        ROW('team_members', 'count', 25)::feature_definition,
        ROW('monthly_sales', 'count', -1)::feature_definition,
        ROW('advanced_analytics', 'boolean', 1)::feature_definition,
        ROW('export', 'boolean', 1)::feature_definition,
        ROW('team_chat', 'boolean', 1)::feature_definition,
        ROW('custom_competitions', 'boolean', 1)::feature_definition,
        ROW('api_access', 'boolean', 1)::feature_definition,
        ROW('advanced_hierarchy', 'boolean', 1)::feature_definition,
        ROW('multiple_teams', 'boolean', 1)::feature_definition,
        ROW('custom_reports', 'boolean', 1)::feature_definition,
        ROW('sso', 'boolean', 1)::feature_definition,
        ROW('advanced_permissions', 'boolean', 1)::feature_definition,
        ROW('sales_forecasting', 'boolean', 1)::feature_definition,
        ROW('crm_integration', 'boolean', 1)::feature_definition
      ]
    WHEN 'enterprise' THEN 
      ARRAY[
        ROW('team_members', 'count', -1)::feature_definition,
        ROW('monthly_sales', 'count', -1)::feature_definition,
        ROW('advanced_analytics', 'boolean', 1)::feature_definition,
        ROW('export', 'boolean', 1)::feature_definition,
        ROW('team_chat', 'boolean', 1)::feature_definition,
        ...
        ROW('sales_forecasting', 'boolean', 1)::feature_definition,
        ROW('crm_integration', 'boolean', 1)::feature_definition,
        ROW('custom_development', 'boolean', 1)::feature_definition,
        ROW('on_premise', 'boolean', 1)::feature_definition,
        ROW('custom_analytics', 'boolean', 1)::feature_definition,
        ROW('custom_integrations', 'boolean', 1)::feature_definition,
        ROW('training', 'boolean', 1)::feature_definition,
        ROW('sla', 'boolean', 1)::feature_definition
      ]
  END;
  
  -- Return features as a table
  RETURN QUERY 
  SELECT 
    f.feature,
    f.limit_type,
